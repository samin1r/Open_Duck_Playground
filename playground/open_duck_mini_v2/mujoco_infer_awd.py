...
class MjInfer:
    def __init__(self, model_path: str, onnx_model_path: str):
        ...
        self.action_filter = LowPassActionFilter(50, 37.5)
    ...
    def get_obs(
        ...
        #         (self.joint_positions - self.init_pos) * self.joint_pos_scale,  # 16
        #         self.joint_velocities * self.joint_vel_scale,  # 16
        #         self.angular_velocity * self.angular_vel_scale_obs,  # 3
        #         self.feet_contact,  # 2
        #     ]
        # )
        obs = np.concatenate(
        ...
        obs = np.concatenate(
            [self.obs_history.flatten(), self.action_history.flatten(), np.array(command[:3])]
        ).reshape(1, -1)
...
if __name__ == "__main__":
    ...
    mjinfer = MjInfer(args.model_path, args.onnx_model_path)
    mjinfer.run()
